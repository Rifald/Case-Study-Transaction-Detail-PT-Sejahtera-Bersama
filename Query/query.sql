...
FROM (productCategory INNER JOIN products ON productCategory.CategoryID = products.Category) INNER JOIN (customers INNER JOIN orders ON customers.CustomerID = orders.CustomerID) ON products.ProdNumber = orders.ProdNumber
